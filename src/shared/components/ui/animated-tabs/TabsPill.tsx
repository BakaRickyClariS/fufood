...
const TabsPill = ({ 
  ...
  return (
    <div className={`flex items-center justify-between bg-neutral-200 rounded-full gap-0.5 p-0.5 relative ${className}`}>
      ...
      {tabs.map((tab, index) => (
        <React.Fragment key={tab.id}>
          <div className="flex items-center justify-center flex-1 z-10">
            <button
              ...
              className={`
                ...
                ${
                  activeTab === tab.id
                    ? (!animated ? 'bg-white shadow-[0_6px_14px_-2px_rgba(0,0,0,0.12)]' : '') + ' text-neutral-900'
          ...
          {index < tabs.length - 1 && (
            <span className="text-neutral-100 font-light text-sm relative z-0">|</span>
          )}
        </React.Fragment>
      ))}
    </div>
