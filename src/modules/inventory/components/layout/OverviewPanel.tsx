...
const OverviewPanel: React.FC<OverviewPanelProps> = ({ onOpenCategory }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 類別數量從 inventoryData (TanStack Query) 計算，這裡只需要取得初始值
        // 實際數量會在下方的 useMemo 中動態計算
  ...
  // 使用 inventoryData (TanStack Query) 動態計算類別數量
  // 這會在消耗操作後自動更新
  const categoriesWithUpdatedCounts = useMemo(() => {
    ...
    return sortedCategories.map((cat) => ({
      ...cat,
      count: currentCounts[cat.id] ?? 0,
    }));
  }, [sortedCategories, inventoryData]);
  ...
  // 例如：layout 格子 0 對應 categoriesWithUpdatedCounts[0]
  const slotToCategoryMap = useMemo(() => {
    const map: Record<string, CategoryInfo | undefined> = {};
    layoutSlots.forEach((slotId, index) => {
      // 將第 index 個格子對應到排序後的第 index 個類別（含更新後的數量）
      map[slotId] = categoriesWithUpdatedCounts[index];
    });
    return map;
