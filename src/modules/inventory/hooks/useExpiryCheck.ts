...
export const useExpiryCheck = (item: FoodItem) => {
  return useMemo(() => {
    ...
    else if (item.lowStockAlert && item.quantity <= item.lowStockThreshold) status = 'low-stock';
  ...
  }, [item.expiryDate, item.quantity, item.lowStockAlert, item.lowStockThreshold]);
};
