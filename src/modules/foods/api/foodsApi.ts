...
export const foodsApi = {
  getCategoryFoods: (catId: string) => apiClient.get<Food[]>(`/foods/category/${catId}`),
  getFoodDetail: (catId: string, id: string) => apiClient.get<Food>(`/foods/category/${catId}/${id}`),
  createFood: (data: Omit<Food, 'id'>) => apiClient.post<Food>('/foods', data),
  updateFood: (id: string, data: Partial<Food>) => apiClient.put<Food>(`/foods/${id}`, data),
