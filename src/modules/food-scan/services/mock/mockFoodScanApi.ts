...
export const createMockFoodScanApi = (): FoodScanApi => {
  ...
  const getFoodItems = async (filters?: FoodItemFilters): Promise<FoodItem[]> => {
    ...
    if (filters?.category) {
      items = items.filter((item: FoodItem) => item.category === filters.category);
    }
    
    if (filters?.isExpiringSoon) {
      const threeDaysLater = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000);
      items = items.filter((item: FoodItem) => new Date(item.expiryDate) <= threeDaysLater);
    }
    
    if (filters?.isLowStock) {
      items = items.filter((item: FoodItem) => 
