...
type ScanResultEditFormProps = {
  ...
  currentUnit?: string; // AI-provided unit that may not be in predefined list
  onRetake?: () => void;
  onPickImage?: () => void;
};
...
const ScanResultEditForm: React.FC<ScanResultEditFormProps> = ({
  ...
  // Build unit options: AI value at top (if exists), then base options (excluding duplicate)
  const unitOptions = currentUnit
    ? [
        { value: currentUnit, label: currentUnit },
        ...baseUnitOptions.filter((opt) => opt.value !== currentUnit),
      ]
    : baseUnitOptions;
