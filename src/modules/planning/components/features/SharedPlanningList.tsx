...
export const SharedPlanningList = ({ statusFilter }: SharedPlanningListProps) => {
  const { lists, isLoading } = useSharedLists();
    ...
    <div className="relative pb-24">
      {filteredLists.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-neutral-400">
          <p>目前沒有{statusFilter === 'in-progress' ? '進行中' : statusFilter === 'pending-purchase' ? '待採買' : '已完成'}的清單</p>
        </div>
      ) : (
